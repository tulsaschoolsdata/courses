  ...
  const renderMenuOptionsForCategory = (category) => {
    ...
    return (
      <div key={schoolCategory[0]}>
        <ListSubheader key={schoolCategory[0]}>
          {schoolCategories[schoolCategory[0]]}
        </ListSubheader>
        {schoolCategory[1].map((school) => (
          <MenuItem
            key={school.school_name}
            value={school.school_id}
            onClick={(_) => handleSchoolChange(school.school_id)}
          >
  ...
  return (
    <Stack spacing={1} sx={{ p: 2, paddingTop: '6em' }}>
      ...
      <FormControl fullWidth>
        ...
        >
          {Object.keys(schoolCategories).map((category) =>
