    ...
    <Stack spacing={2} sx={{ p: 2, paddingTop: '6em' }}>
      ...
      {!hideSchools && (
        <FormControl fullWidth>
          ...
          >
            {Object.entries(schoolCategories).filter((category) => ["Middle", "High"].includes(category[1])).map((category) =>
              renderMenuOptionsForCategory(category)
            )}
          </Select>
        </FormControl>
      )}
      ...
      {/* <FormControl fullWidth>
        ...
        <Select sx={{ maxHeight: 500 }} label="Core" value={filters.is_core}>
          <MenuItem
            key={'Yes'}
            value={true}
            onClick={(_) => handleChange('is_core', true)}
          >
            Yes
          </MenuItem>
          <MenuItem
            key={'No'}
            value={false}
            onClick={(_) => handleChange('is_core', false)}
          >
            No
          </MenuItem>
        </Select>
        <FormHelperText>Only Include Core Courses?</FormHelperText>
      </FormControl>
      <FormControl fullWidth>
        <InputLabel>Vocational</InputLabel>
        <Select
          sx={{ maxHeight: 500 }}
          label="Vocational"
          value={filters.is_vocational}
        >
          <MenuItem
            key={'Yes'}
            value={true}
            onClick={(_) => handleChange('is_vocational', true)}
          >
            Yes
          </MenuItem>
          <MenuItem
            key={'No'}
            value={false}
            onClick={(_) => handleChange('is_vocational', false)}
          >
            No
          </MenuItem>
        </Select>
        <FormHelperText>Only Include Vocational Courses?</FormHelperText>
      </FormControl> */}
      <Button
        variant="contained"
        color="primary"
