...
export default function Filters({
  ...
  const getSchoolName = (schoolId) => {
    return schools.map(school => school[1]).flatMap(s => s).find((s) => s.school_id == schoolId).school_name
  }
  ...
  return (
    <Stack spacing={1} sx={{ p: 2, paddingTop: '6em' }}>
        ...
        <Select
          ...
          renderValue={(selected) =>
            selected.map((schoolId) => (
              <Chip key={schoolId} sx={{ marginRight: 0.5 }} label={getSchoolName(schoolId)} />
            ))
          }
        >
          {schools.map((schoolCategory) => (
            <div key={schoolCategory[0]}>
              <ListSubheader key={schoolCategory[0]}>{schoolCategory[0]}</ListSubheader>
              {schoolCategory[1].map(school => (
                <MenuItem key={school.school_name} value={school.school_id} onClick={_ => handleSchoolChange(school.school_id)}>
                  <Checkbox checked={isChecked('schools', school.school_id)} />
                  {school.school_name}
                </MenuItem>
              ))}
            </div>
      ...
      <Button variant="contained" color="primary" onClick={() => setFiltersOpen(false)}>Done</Button>
